...
export const ChatMessage: React.FC<ChatMessageProps> = ({ message }) => {
  return (
    ...
    >
      ...
      <div
        className={`md:max-w-[80%] max-w-[94%] rounded-lg px-4 py-2 text-sm shadow ${
          message.role === "user"
      ...
      >
        <div className="space-y-3">
          {message.role === "user" && <p>{message.parts[0].text}</p>}

          {message.answer && <ReactMarkdown>{message.answer}</ReactMarkdown>}
